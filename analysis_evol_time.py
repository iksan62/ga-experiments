...
TARGET_METRIC = 'time_for_ga_process'
...
# -------------------------------------------------------------------
# 2. Preprocessing - take time_for_ga_process
# -------------------------------------------------------------------
...
summary_stats_df.rename(columns={
    'mean': 'Mean_GA_Process',
    'std': 'Std_GA_Process'
}, inplace=True)

summary_stats_df.to_csv(
    os.path.join(output_dir, f"2a_mean_std_dev_{TARGET_METRIC}.csv"),
    index=False
)
...
pivot_df.to_csv(
    os.path.join(output_dir, f"3_pivot_table_for_analysis_{TARGET_METRIC}.csv")
)
...
with open(
    os.path.join(output_dir, f"4_friedman_test_result_{TARGET_METRIC}.txt"),
    'w'
) as f:
...
if p < 0.05:
    ...
    nemenyi_df.to_csv(
        os.path.join(output_dir, f"5_nemenyi_test_results_{TARGET_METRIC}.csv")
    )
...
try:
    ...
    with open(
        os.path.join(output_dir, f"6_autorank_report_{TARGET_METRIC}.txt"),
        'w'
    ) as f:
        f.write(autorank_report if autorank_report else "Autorank report is empty.\n")

    result.rankdf.to_csv(
        os.path.join(output_dir, f"7_autorank_rankings_{TARGET_METRIC}.csv")
    )
    ...
    plt.savefig(
        os.path.join(output_dir, f"8_cd_diagram_{TARGET_METRIC}.svg"),
        dpi=300,
        bbox_inches='tight'
    )
...
# -------------------------------------------------------------------
avg_time_value = summary_stats_df.groupby('methods')['Mean_GA_Process'].mean()
...
with open(
    os.path.join(output_dir, f"10_summary_results_{TARGET_METRIC}.txt"),
    'w'
) as f:
