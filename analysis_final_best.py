...
output_dir = r"D:\output_analysis_final_best"
...
# -------------------------------------------------------------------
# 2. Preprocessing & Get Global Best Objective Value
# -------------------------------------------------------------------
print("\nFinding best objective value (global best) for each run...")
run_cols = [f'run-{i}' for i in range(1, 21)]
group_cols = ['benchmark', 'methods']

# Group by benchmark and methods, then take minimum value for each run
global_best_df = df.groupby(group_cols)[run_cols].min().reset_index()
global_best_df.to_csv(os.path.join(output_dir, "2_data_global_best.csv"), index=False)
print("Global Best data successfully extracted and saved.")

# Compute mean and std dev from 20 best values
...
summary_stats_df.to_csv(os.path.join(output_dir, "2a_mean_std_dev_table_global_best.csv"), index=False)
print("\nMean and Std Dev table for Global Best saved.")
...
pivot_df.to_csv(os.path.join(output_dir, "3_pivot_table_for_analysis_global_best.csv"))
...
# -------------------------------------------------------------------
print("\nPreparing raw data for Autorank...")

# Convert data to long format: benchmark, methods, run, value
long_df = df.melt(id_vars=['benchmark', 'methods'], 
                  value_vars=run_cols, 
                  var_name='run', 
...
pivot_runs_df.to_csv(os.path.join(output_dir, "3b_pivot_table_runs_for_autorank.csv"))
...
with open(os.path.join(output_dir, "4_friedman_test_result_global_best.txt"), 'w') as f:
...
if p < 0.05:
    ...
    nemenyi_df.to_csv(os.path.join(output_dir, "5_nemenyi_test_results_global_best.csv"))
...
try:
    ...
    with open(os.path.join(output_dir, "6_autorank_report_global_best.txt"), 'w') as f:
        f.write(autorank_report if autorank_report else "Autorank report is empty.\n")

    result.rankdf.to_csv(os.path.join(output_dir, "7_autorank_rankings_global_best.csv"))

    # CD Diagram
    plt.figure(figsize=(10, max(2, len(methods) * 0.5)))
    plot_stats(result)
    plt.title("Critical Difference Diagram (Global Best Objective)", fontsize=14)
    ...
    plt.savefig(os.path.join(output_dir, "8_cd_diagram_global_best.svg"), dpi=300, bbox_inches='tight')
    plt.close()
...
best_method_text = f"\nMethod with lowest average 'global best objective value' = {best_method}\n"
print(best_method_text)

summary_text = "--- Statistical Analysis Summary (Global Best Objective) ---\n\n"
summary_text += "== Friedman Test Result ==\n"
summary_text += friedman_result_text
summary_text += "\n== Best Method ==\n"
summary_text += best_method_text

with open(os.path.join(output_dir, "10_summary_results_global_best.txt"), 'w') as f:
